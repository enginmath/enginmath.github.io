...
1. **下三角方位**:原写"右下部分",应为**左下**;且上下三角元素个数相同,
   无"大/小"之分
2. **6 → 5 的理由**:不是"减去已知的 $n$",而是 **Hankel 结构使 (1,3) 与 (2,2)
   重复**
3. **"n=4 → 10"**:4×4 对应 **3 次拟合**(非 4 阶);独立数值是 **7** 个
   ($2m+1$),不是 10;$\frac{n(n+1)}{2}$ 是一般对称矩阵的公式,不适用于 Hankel
4. **"n 被完全消了"**:$n$ 仍在 $A^{\mathsf T}A$ 右下角;消失的是**矩阵尺寸对
   $n$ 的依赖**
5. **系数下标错位**:$p(x)=c_0+c_1x+c_2x^2$(升幂)与降幂的 $A$ 不匹配;
   全文改为降幂
6. **$Ac=y$ 写成等号**:在讲超定的语境里应为 $\approx$
7. **RLS 术语**:"8 累加器 + 重解"应叫「正规方程的在线累加」;RLS 特指
   Sherman–Morrison 增量更新

## C.2 补上的逻辑缺口

8. **凸性**:$A^{\mathsf T}A$ 半正定 ⇒ $E$ 凸 ⇒ 驻点即全局最小(§6.1)
9. **可解条件**:$A$ 列满秩,即互不相同的 $x$ 至少 $m+1$ 个(§6.2)
10. **矩阵求导规则**:写出所用的两条,并指出第二条依赖 $M$ 对称(§4.5)
11. **伪逆前提**:$A^{+}=(A^{\mathsf T}A)^{-1}A^{\mathsf T}$ 需列满秩(§10)
12. **消元过程**:例题补全从三个方程到 $c=[1,1,1]$ 的推导(§9.4)
13. **数值警告**:$\sum x^4$ 的精度问题与归一化对策(§7.3)

## C.3 一致性与结构

14. 残差统一为 $r=y-Ac$(原文两处符号相反)
15. 未知数统一为 $c_0,c_1,c_2$(原文 $a,b,d$;另有一处误用 $\theta$)
16. 转置证明中的占位符改为 $P,Q,R$(原文用 $A$ 同时表示占位符和真实矩阵)
17. 措辞:"$A$ 通过转置" → "**左乘** $A^{\mathsf T}$"
18. "范德蒙德矩阵"加注说明:高瘦的应称**设计矩阵**,行列式公式仅对方阵成立
19. **最小二乘一节前移**至两次推导之前(它是地基)
20. **几何视角独立成节**(原文散落在多处)
21. 章节编号统一(原文"三个问题"编号 3、4、5 与章节 1、2、3 对不上)
22. 中文全部移出 `$$...$$` 数学环境(原文十余处,渲染会变斜体并吞空格)
23. 清理重复段落(ESP32 说了两遍、同一方程贴了两遍)与零碎 typo
    (`TA:`、`bi si jie`、漏等号、断行错位等)